...
On the embedded model-call path, the router also records sanitized `model_call_started` / `model_call_ended` events with the selected target and the effective provider/model per call. Native Codex may additionally expose trusted `model.call.*` diagnostics, but OpenClaw marks them `observationUnit: "turn"`; the router records those with `resolved*` fields and synthetic turn `callId` semantics. **They do not currently verify native Codex provider calls per hidden request, retry, or fallback**: `agent_end` and Codex adapter-level lifecycle observations are not equivalent to provider-call telemetry. See [effective-model observability](docs/model-observability.md) for the runtime-specific guarantee and the open issue #2 gate.
